...
//                               "C:\\Users\\5rota\\Desktop\\SuperDataBase.db"
                               "C:\\Users\\Rota5\\Documents\\2\\hackathonKindCamp\\protocol\\Server\\sql\\SuperDataBase.db",
//                               "C:\\Users\\5Rota\\Documents\\hackathonKindCamp\\protocol\\Server\\sql\\SuperDataBase.db",
                           });
...
QList<QObject*> ServerController::getLocationLog(QString login)
{
    QString str = QString("select datetime, latitude, longitude from location_log where login='%0'").arg(login);
    QSqlQuery query(str);
    QList<QObject*> logs;
    while(query.next()){
        logs.append(new LocationLog(login, getDateTime(svalue(0)), query.value(1).toDouble(), query.value(1).toDouble()));
    }
    return logs;
}

QList<QObject*> ServerController::getLocationLog(QString login, QDateTime from, QDateTime to)
{
    ...
    QList<QObject*> logs;
    while(query.next()){
        logs.append(new LocationLog(login, getDateTime(svalue(0)), query.value(1).toDouble(), query.value(2).toDouble()));
    }
    return logs;
}

QList<QObject*> ServerController::getSchedule(QString login)
{
    ...
    QList<QObject*> sc;
    while(query.next()){
        sc.push_back(new ScheduleElement(ivalue(0), svalue(1), getDateTime(svalue(2)), svalue(3),svalue(4).toDouble(), svalue(5).toDouble()));
    }
...
Child* ServerController::getChildInfo(QString login)
{
    ...
    Child* c=new Child(name, avatar, info, bd, house, room, party_name, owner_name, cash, from, to);
...
Child* ServerController::getFullChildInfo(QString login)
{
    ...
    ((AdditionalChildInfo*)answer->add_info)->climat = climat;
    ((AdditionalChildInfo*)answer->add_info)->diet = diet;
    ((AdditionalChildInfo*)answer->add_info)->doctor = new Doctor(doctor_name, doctor_specialization, doctor_room, workgraph);
    ((AdditionalChildInfo*)answer->add_info)->excursion = excursion;
    ((AdditionalChildInfo*)answer->add_info)->healt_group = healt_group;
    ((AdditionalChildInfo*)answer->add_info)->main_diagnoz = main_diagnoz;
    ((AdditionalChildInfo*)answer->add_info)->second_diagnoz = second_diagnoz;
    ((AdditionalChildInfo*)answer->add_info)->min_water = min_water;
    ((AdditionalChildInfo*)answer->add_info)->move_mode = move_mode;
    ((AdditionalChildInfo*)answer->add_info)->organization = organization;
    ((AdditionalChildInfo*)answer->add_info)->sport_games = sport_games;
    ((AdditionalChildInfo*)answer->add_info)->yfz = yfz;
    return answer;
}

QList<QObject*> ServerController::getSchedule(QString login, QDateTime from, QDateTime to)
{
...
QList<QObject *> ServerController::getTransactions(QString login)
{
    QString str = QString("SELECT datetime, value, description FROM cash_history where clogin = '%1';").arg(login);
    QSqlQuery query(str);
    QList<QObject*> ts;
    while(query.next()){
        ts.push_back(new Transaction(ivalue(1), svalue(2), getDateTime(svalue(0))));
    }
