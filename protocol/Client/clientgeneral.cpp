...
void ClientGeneral::handlerAuthorization(QJsonObject *object)
{
    ...
    if(resultStatus == ProtocolCommunication::___STS_DONE)
    {
        ...
        {
            sendGetUserInfoKid();
//            sendGetScheduleOnTodayForKid();
        }
        else if(role == 2) //родитель
        {
            sendGetUserInfoParent(myLogin);
//            sendGetScheduleOnTodayForParent();
        }
    }
    else
    {
...
void ClientGeneral::handlerInfoUserKid(QJsonObject *object)
{
    ...
    person = new Child(name, image, description, birthDate, house, room, partyName, ownerName, cash.toInt(), from, to);
...
void ClientGeneral::handlerInfoUserParent(QJsonObject *object)
{
    ...
    child = new Child(name, ProtocolCommunication::StringToByteArray(image), description, birthDate, house, room, partyName, ownerName, cash.toInt(), from, to);

    ((AdditionalChildInfo*)child->add_info)->diet = ((*object)[ProtocolCommunication::___DIET]).toString();
    ((AdditionalChildInfo*)child->add_info)->excursion = ((*object)[ProtocolCommunication::___EXCURSION]).toString();
    ((AdditionalChildInfo*)child->add_info)->move_mode = ((*object)[ProtocolCommunication::___MOVE_MODE]).toString();
    ((AdditionalChildInfo*)child->add_info)->min_water = ((*object)[ProtocolCommunication::___MIN_WATER]).toString();
    ((AdditionalChildInfo*)child->add_info)->yfz = ((*object)[ProtocolCommunication::___YFZ]).toString();
    ((AdditionalChildInfo*)child->add_info)->sport_games = ((*object)[ProtocolCommunication::___SPORT_GAMES]).toString();
    ((AdditionalChildInfo*)child->add_info)->climat = ((*object)[ProtocolCommunication::___CLIMAT]).toString();
    ((AdditionalChildInfo*)child->add_info)->main_diagnoz = ((*object)[ProtocolCommunication::___MAIN_DIAGNOZ]).toString();
    ((AdditionalChildInfo*)child->add_info)->second_diagnoz = ((*object)[ProtocolCommunication::___SECOND_DIAGNOZ]).toString();
    ((AdditionalChildInfo*)child->add_info)->organization = ((*object)[ProtocolCommunication::___ORGANIZATION]).toString();
    ((Doctor*)((AdditionalChildInfo*)child->add_info)->doctor)->name = ((*object)[ProtocolCommunication::___DOCTOR_NAME]).toString();
    ((AdditionalChildInfo*)child->add_info)->healt_group = ((*object)[ProtocolCommunication::___HEALT_GROUP]).toString();
...
QString ClientGeneral::getName()
{return getCurrentChild()->name;}

QString ClientGeneral::getAvatar()
{return getPathFile(getCurrentChild()->avatar);}

QString ClientGeneral::getPathFile(QByteArray file)
{
    if(images.contains(file))
        return images[file];

    QString loc=QStandardPaths::locate(QStandardPaths::DownloadLocation,"", QStandardPaths::LocateDirectory);
    qDebug() << loc;
    loc+="garbage/";
    if(!QDir(loc).exists()){
        QDir df(loc);
    ...
    QString fileName=QString::number(QDateTime::currentDateTime().time().msecsSinceStartOfDay());
    qDebug() << fileName;
    ...
    images[file]="file:///"+loc+fileName;
    return images[file];

}

int ClientGeneral::getLvl()
{
    return 5;
}

QString ClientGeneral::getBirthDate()
{
 return getCurrentChild()->birthdate.toString();
}
